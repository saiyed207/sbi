...
$accountNumber = $_SESSION['account_number']; // Assuming you are passing this value from the frontend
$pin = $_SESSION['pin']; // Assuming you are passing this value from the frontend
...
if ($result->num_rows > 0) {
    // Account number and PIN are correct, fetch the account details
    $row = $result->fetch_assoc();
    $account_balance = $row['account_balance'];
    $account_holder_name = $row['account_holder_name'];
    $account_number = $row['account_number'];

    // Store the fetched values in the session
    $_SESSION['account_number'] = $account_number;
    $_SESSION['account_holder_name'] = $account_holder_name;
} else {
    // Account number or PIN is incorrect, show failure message
    echo "failure";
}
...
$("#bac4").click(function(){
    ...
    $(document).ready(function() {
            // Function to validate PIN using AJAX
 ...
 $(document).ready(function() {
            $("#backs").click(function() {
                ...
                $.ajax({
                    ...
                    url: "logout.php",
                    success: function(response) {
                        // The logout action is complete (handle the response if needed)
                    },
                    error: function(xhr, status, error) {
                        // Error occurred during logout (handle the error if needed)
                    }
                });
            });
        });
...
$("#back5").click(function(){
    ...
    $("#btns2").click(function() {
        ...
        $.ajax({
            type: "POST",
            url: "withdraw.php", // Update the URL with the correct path to your PHP script
            data: { withdrawAmount: withdrawAmount }, // Pass the withdrawal amount to the server
            success: function(response) {
                $("#wdt3").empty();
                $("#wdt5").empty();
                $("#wdt7").empty();
                ...
                $("#indicator2").hide();
                 ...
                 // Replace this with the actual account balance from the server if needed
